...
#include "secrets.h"   // WiFi + Firebase credentials (gitignored — never commit)
...
// Snapshot buffer for countTipsInWindow — global to avoid 800-byte stack allocation per call
static unsigned long tipSnapshot[MAX_TIPS];
...
float countTipsInWindow(unsigned long windowMs) {
  ...
  for (int i = 0; i < localCount; i++) tipSnapshot[i] = tipTimestamps[i];
  interrupts();

  unsigned long now = millis();
  int count = 0;
  for (int i = 0; i < localCount; i++) {
    if ((now - tipSnapshot[i]) <= windowMs) count++;
  }
